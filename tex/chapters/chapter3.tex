...
However, the computational cost of solving this equation directly scales exponentially with the size of the matrix $\mathbf{A}$. 
...
In many schemes, the eigenvalues and eigenvectors are revealed through the application of sequences of matrices that are similar to $\mathbf{A}$. 
Eventually, these sequences converge to a triangular or diagonal form, which allows the eigenvalues to be read easily. 
...
With these definitions, Equation \ref{eq:keig} may be rewritten as a general eigenvalue problem, i.e.,
\begin{equation}
...
Then, the $\mathbf{x}$ can be updated by solving a typical $\mathbf{A} \mathbf{x} =\mathbf{b}$ problem, where $\mathbf{b}=\frac{1}{k}\mathbf{B}\mathbf{x}$ is a vector.
...
This process is repeated until convergence criteria are met, which produces the dominant eigenmodes at the steady state.
The algorithm for the full power method is as follows:
\begin{enumerate}
...
This approach can be simply implemented in the programming language of choice.
...
\end{split}
\end{equation}
Without loss of generality, let $c_0' = 1$.
Application of operator $\mathbf{A}$ leads to 
\begin{equation}
\begin{split}
  \mathbf{A} \mathbf{x}^{(0)} 
...
\end{split}
\end{equation}
Repeated applications of operator $\mathbf{A}$ leads to
\begin{equation}
\begin{split}
