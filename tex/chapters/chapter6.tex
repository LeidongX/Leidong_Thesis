...
\end{equation}
where the notations are consistent as \CHAPTER{chapter:multigroup}.
...
The basic core layout is shown in \FIGURE{fig:IAEA},
...
Also, the the well-studied 2-D C5G7 benchmark were analyzed \cite{oecd_nuclear_energy_agency_benchmark_2003}, in order to verify the performance of algorithm for multi-dimensional problems.
The configure of the benchmark is adapted from the benchmark problem presented in \citet{reed_2015}.
The configure of a quarter core contains four fuel pin assemblies and five moderator.assemblies is shown in \FIGURE{fig:C5G7_config}.
Each fuel assembly has 17 $\times$ 17 individual pincells, and the configuration of $UO_2$ assemblyis shown in \FIG{fig:UO2_config} and the configuration of MOX assembly is shown in \FIG{fig:MOX_config}.
Here, each pincell is discretized on a 7$\times$ Cartesian mesh. The demensions are shown in \FIG{fig:MOX_config}.
\begin{figure*}[htb]
    \centering
...
Note that the final result reaching the lower bound here is very closed to the direct solution predicted by only the dominant DMD modes $\mathbf{\Phi_0}$, which demonstrate that our modified \EQUATION{eq:f_mode} can produce the results in a same level of accuracy.
...
In this case, a tolerance of $10^{-14}$ is used as the symbol of convergence.
...
The error is defined as same as \EQUATION{eq:flatten}, and the tolerance of convergence is set as $10^{-8}$.
